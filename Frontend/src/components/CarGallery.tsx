    ...
    <div>
      ...
      <div className="mt-3 grid grid-cols-6 gap-2">
        {images.map((image, index) => (
          <button key={image.url} onClick={() => setActive(index)} aria-label={`Фото ${index + 1}`} className={`relative aspect-square overflow-hidden rounded-xl border-2 ${index === active ? "border-primary" : "border-gray-border"}`}>
