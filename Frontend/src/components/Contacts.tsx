...
const Contacts = ({ hideHeading = false }: { hideHeading?: boolean }) => {
  return (
    <section id="contacts" className="bg-gray-bg py-14 sm:py-20 lg:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {!hideHeading && <SectionHeading eyebrow="Контакты" title="Свяжитесь с нами" description="Ответим на любые вопросы и поможем подобрать автомобиль" className="mb-9 sm:mb-14" />}

        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          <div className="h-full">
            <div className="h-full rounded-2xl border border-gray-border bg-white p-6 sm:p-8 lg:p-10">
              <ContactList />
            </div>

          </div>

          <div className="rounded-2xl border border-gray-border bg-white p-6 sm:p-8 lg:p-10">
            <ContactForm />
          </div>
        </div>
        <SectionHeading eyebrow="Где нас найти" title="Местоположение" className="mb-7 mt-12 sm:mb-10 sm:mt-16" />
        <div className="grid overflow-hidden rounded-[20px] border border-gray-border bg-white lg:grid-cols-2">
          ...
          <iframe src="https://yandex.ru/map-widget/v1/?text=Тольятти%2C%20Офицерская%20улица%2C%2046&z=16" className="h-full min-h-[320px] w-full self-stretch lg:min-h-[420px]" title="Карта — расположение автосалона TIGLEV.COM в Тольятти" loading="lazy" />
        </div>
      </div>
    </section>
  );
