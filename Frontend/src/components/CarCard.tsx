  ...
  return (
    <article onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className="group relative flex h-full flex-col overflow-hidden rounded-xl border border-gray-border bg-white sm:rounded-2xl">
      ...
      <div className="pointer-events-none relative z-20 aspect-[16/10] overflow-hidden bg-gray-bg">
        ...
        <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); stepImage(1); }} aria-label={`Следующее фото ${car.brand} ${car.model}`} className="pointer-events-auto absolute right-0 top-1/2 z-30 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-transparent opacity-100 transition-opacity hover:[&>span]:bg-dark/65 focus-visible:[&>span]:bg-dark/65 active:[&>span]:scale-95 sm:right-1 md:opacity-0 md:group-hover:opacity-100 md:focus:opacity-100"><span className="flex h-8 w-8 items-center justify-center rounded-full border border-white/20 bg-dark/35 text-white/95 backdrop-blur-sm transition-[background-color,transform]"><ChevronRight className="h-3.5 w-3.5"/></span></button></>}
